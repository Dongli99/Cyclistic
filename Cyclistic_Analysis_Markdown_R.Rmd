...
*A [Kaggle version](https://www.kaggle.com/code/dongliai/cyclistic-bike-share-a-case-study) is much easier to recurrent.*
...
To download multiple files automatically, the [mutidownloader Python app](https://github.com/Dongli99/MultiDownloader.git) was developed under the assistant of ChatGPT.
This case study selects the subset of the data (2020Q1-202307).

> Data Overview

  - location: root/tripdata
  - organization: 202003 include Q1 data, others are organized by month.
  - data size: 3.3G
  - num of files: 41
  - num of columns: 13
...
- This task is to select and optimize one geographic data for future analysis
...
- Seeking probability of alternatively using lng & lat.
...
lat_lng_list <- tb %>% 
  mutate(
    ...
    end_lng = round(end_lng, digits = 3)# str(lat_lng_list)
  ) %>% 
...
- according to the number of rows `digit=2: 749; digit=3: 4878`, lat&lng is not good representative.
- Before using station name, check the distribution of empty name.
